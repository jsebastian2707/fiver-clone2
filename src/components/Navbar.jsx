          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between h-20">
              <Link to="/" className="flex items-center space-x-2">
                ...
                <span className="text-2xl font-bold tracking-tight">HorizonteProfesional</span>
              </Link>
                ...
                {user ? (
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent className="w-56 mr-4 mt-2 glassmorphism border-purple-400">
                      ...
                      <DropdownMenuItem onClick={() => navigate('/profile')} className="hover:bg-purple-500/20">
                        <User className="mr-2 h-4 w-4 text-purple-400" />
                        <span>Perfil</span>
                      </DropdownMenuItem>
                      {user?.isAdmin && (
                        <DropdownMenuItem onClick={() => navigate('/admin')} className="hover:bg-purple-500/20">
                          <LayoutDashboard className="mr-2 h-4 w-4 text-purple-400" />
                          <span>Panel Admin</span>
                        </DropdownMenuItem>
                      )}
                      <DropdownMenuSeparator className="bg-purple-300"/>
                      <DropdownMenuItem onClick={handleLogout} className="text-red-400 hover:!text-red-400 hover:!bg-red-500/20">
                  ...
                  <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                    <Button onClick={() => navigate('/auth')} variant="secondary" className="bg-yellow-400 hover:bg-yellow-500 text-purple-700 font-semibold">
