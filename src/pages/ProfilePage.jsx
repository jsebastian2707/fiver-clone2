...
const ProfilePage = () => {
  ...
  const handleSaveProfile = async () => {
    ...
    toast.success('Perfil actualizado', { description: 'Tu información fue guardada correctamente.' });
  ...
  //       JSON.parse(localStorage.getItem(`userServices_${user.id}`)) || [];
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      if (data) {
        ...
        // Si no existe perfil aún, usar valores por defecto
        setProfileData({
          name: user.email.split("@")[0],
          email: user.email,
          profession: "",
          description: "",
          avatarUrl: `https://api.dicebear.com/6.x/initials/svg?seed=${
            user.email || "U"
          }`,
        });
      }
  ...
  const handleSaveService = () => {
    ...
    localStorage.setItem(
      `userServices_${user.id}`,
      JSON.stringify(updatedServices)
    );
    toast({
      title: "Servicio Guardado",
      description: `El servicio "${currentService.name}" ha sido guardado.`,
    });
  ...
  return (
    <motion.div
      ...
      className="space-y-8 max-w-5xl w-full px-4 sm:px-6 lg:px-8 py-8"
    >
      <Card className="w-full glassmorphism border-purple-500/50 shadow-xl shadow-purple-500/10">
          ...
          <motion.div whileHover={{ scale: 1.1 }} className="relative">
            ...
            {isEditingProfile && (
              <Button
                size="icon"
                variant="outline"
                className="absolute bottom-0 right-0 bg-gray-800 hover:bg-gray-700 border-yellow-400 text-yellow-400 rounded-full h-8 w-8"
              >
                <Edit3 size={16} />
              </Button>
            )}
          ...
          <div>
            {isEditingProfile ? (
              ...
              <Button
                ...
                variant="outline"
                className="border-yellow-400 text-yellow-400 hover:bg-yellow-400 hover:text-purple-700"
              >
        ...
        <CardContent>
          ...
          {isEditingProfile && (
            <div className="mt-4 space-y-2">
              <Label htmlFor="avatarUrl" className="text-indigo-100">
                URL de Avatar
              </Label>
              <Input
                id="avatarUrl"
          ...
          {isAddingService && (
            ...
            >
              <h3 className="text-xl font-semibold text-yellow-200">
                {currentService.id ? "Editar" : "Añadir Nuevo"} Servicio
              </h3>
              <div>
                <Label htmlFor="serviceName" className="text-indigo-100">
                  Nombre del Servicio
                </Label>
                <Input
                  id="serviceName"
                  ...
                  className="bg-purple-900/50 border-purple-700 text-white focus:border-yellow-400"
                />
              </div>
              <div>
                <Label htmlFor="serviceDescription" className="text-indigo-100">
                  Descripción
                </Label>
                <Textarea
              ...
              <div>
                <Label htmlFor="servicePrice" className="text-indigo-100">
                  Precio (ej: 50 USD, 20 EUR/hora)
                </Label>
                <Input
                  id="servicePrice"
                  name="price"
              ...
              <div>
                <Label htmlFor="serviceCategory" className="text-indigo-100">
                  Categoría
                </Label>
                <Input
                  id="serviceCategory"
                  ...
                  onChange={handleServiceChange}
                  placeholder="Ej: Diseño Gráfico, Traducción"
                  className="bg-purple-900/50 border-purple-700 text-white focus:border-yellow-400"
                />
              </div>
              <div className="flex space-x-2">
                ...
                >
                  ...
                  {currentService.id ? "Actualizar" : "Guardar"} Servicio
                </Button>
                <Button
                  variant="outline"
          ...
          {services.length === 0 && !isAddingService ? (
            ...
            <div className="space-y-4">
              {services.map((service) => (
                ...
                >
                  <div>
                    <h4 className="font-semibold text-lg text-yellow-200">
                      {service.name}{" "}
                    ...
                    <p className="text-md font-bold text-green-400 mt-1">
                      {service.price}
                    </p>
                  </div>
